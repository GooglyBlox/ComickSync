(() => {
  ...
  function makeDetectionKey(detection) {
    ...
    const normalizedEpisode =
      detection.episode == null ? "" : String(detection.episode);
  ...
  function clearChapterIndicators() {
    document
      .querySelectorAll('[data-comicksync-read="true"]')
      .forEach((element) => {
  ...
  function createManualSyncConfig(detection, response, options = {}) {
    ...
    return {
      ...
      onSubmit: async (value) => {
        ...
        await performSync(options.adapter, options.runner, detection, {
          episode: normalized,
        });
      },
    };
  ...
  function renderResponseState(
      ...
      setPanelState(
        ...
        [
          ...
          },
          {
            label: "Latest",
            value:
              response.libraryLastChapter != null
                ? `Chapter ${response.libraryLastChapter}`
                : "Latest chapter unavailable",
          },
        ],
        {
          actions: getActionDescriptors(detection, response, {
            adapter,
            runner,
          }),
        },
    ...
    if (response?.reason === "comic_not_found") {
      ...
      setPanelState(
        ...
        [{ label: "Status", value: "No matching series found on Comick" }],
        {
          ...
          manualSync: createManualSyncConfig(detection, response, {
            adapter,
            runner,
          }),
        },
    ...
    if (response?.reason === "chapter_not_found") {
      ...
      setPanelState(
        ...
        [
          {
            label: "Status",
            value: `Series found, but Chapter ${detection.episode} was not found on Comick`,
          },
        ],
        {
          actions: getActionDescriptors(detection, response, {
    ...
    if (response?.reason === "not_authenticated") {
      if (pageType === "sync" && shouldNotify) {
        showToast(
          "ComickSync",
          "Log in to Comick to sync progress",
          `${makeDetectionKey(detection)}:not_authenticated`,
        );
      }
      setPanelState(
        adapter,
    ...
    if (response?.reason === "sync_error" || response?.error) {
      if (pageType === "sync" && shouldNotify) {
        showToast(
          detection.title,
          "Sync failed",
          `${makeDetectionKey(detection)}:sync_error`,
        );
      }
      setPanelState(
        adapter,
        ...
        [
          {
            label: "Status",
            value:
              "This looks like a new series. Sync only if you want to start it on Comick.",
          },
          {
            label: "Chapter",
            value:
              detection?.episode != null
                ? `Chapter ${detection.episode}`
                : "Unknown",
          },
        ],
        {
  ...
  function applyChapterIndicators(runner, response) {
    ...
    for (const element of elements) {
      let episode = null;
      try {
        episode = runner.evaluateFieldWithInput(
          "list",
          "elementEp",
          element,
          false,
      ...
      const isRead =
        !Number.isNaN(numericEpisode) &&
        (readEpisodes.has(numericEpisode) ||
          (hasLibraryProgress && numericEpisode <= libraryEpisode));
      if (!isRead) {
        continue;
      }
  ...
  function updateFloatButton(detection, response) {
      ...
      if (popupMeta)
        popupMeta.textContent =
          response.reason === "comic_not_found"
            ? "Not found on Comick"
            : response.reason === "chapter_not_found"
              ? `Series found, Ch. ${detection.episode} not matched`
              : response.reason === "not_authenticated"
                ? "Not signed in"
    ...
    if (popupActions) {
      ...
      getActionDescriptors(detection, response, { adapter: null, runner: null })
        .slice(0, 3)
        .forEach((action) => {
          const button = document.createElement("button");
          button.type = "button";
    ...
    chrome.runtime.sendMessage(
      ...
      async (adapterResponse) => {
        ...
        if (chrome.runtime.lastError || !adapterResponse?.adapter) {
          log(
            "No matching adapter or runtime error",
        ...
        try {
          ...
          const isSyncPage = Boolean(
            runner.evaluateField("sync", "isSyncPage"),
          );
          const isOverviewPage = Boolean(
            runner.evaluateField("overview", "isOverviewPage"),
          );
          ...
          const overviewUrl = isSyncPage
            ? runner.evaluateField("sync", "getOverviewUrl")
            : url;
          const nextEpUrl = isSyncPage
            ? runner.evaluateField("sync", "nextEpUrl")
            : null;
          const image = runner.evaluateField(pageType, "getImage");
          const volume = isSyncPage
            ? runner.evaluateField("sync", "getVolume")
            : null;
          const episode = isSyncPage
            ? runner.evaluateField("sync", "getEpisode")
            : null;
          ...
          if (isSyncPage) {
            episodeNumber =
              typeof episode === "number" ? episode : Number(episode);
            if (
              episode === null ||
              episode === undefined ||
              ...
              setPanelState(adapter, runner, pageType, "error", title, [
                {
                  label: "Status",
                  value: "Unable to read chapter number from this page",
                },
              ]);
              return;
            }
          ...
          if (
            detectionKey === lastDetectionKey &&
            lastRenderedState?.key === detectionKey
          ) {
            renderResponseState(
              adapter,
              runner,
              detection,
              lastRenderedState.response,
              { notify: false },
            );
            return;
          }
          ...
          const sendDetection = (detectionPayload = detection) => {
            ...
            const messageType = isSyncPage
              ? "SYNC_DETECTION"
              : "RESOLVE_SERIES";
            chrome.runtime.sendMessage(
              { type: messageType, detection: detectionPayload },
              (response) => {
                if (isStale()) {
                ...
                if (
                  ...
                  if (
                    response?.reason === "confirmation_required" &&
                    detectionKey === cancelledDetectionKey
                  ) {
                    clearInFlightIfCurrent(detectionKey);
                    clearPanel();
                    removeFloatButton();
          ...
          setPanelState(
            ...
            "overview",
            "error",
            adapterResponse.adapter.name,
            [
              {
                label: "Status",
                value: `Detection failed: ${error?.message ?? "unknown error"}`,
              },
            ],
          );
        }
