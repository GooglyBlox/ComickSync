...
class CompiledPageRunner {
  ...
  exec(fn, current, args) {
    switch (fn) {
      ...
      case "find":
        return isElementLike(current)
          ? (current.querySelector(String(args[0])) ?? null)
          : null;
      case "findAll":
        return isElementLike(current)
          ? Array.from(current.querySelectorAll(String(args[0])) ?? [])
          : [];
      case "text":
      ...
      case "getAttribute":
        return isElementLike(current)
          ? current.getAttribute(String(args[0]))
          : null;
      case "closest":
        return isElementLike(current)
          ? (current.closest(String(args[0])) ?? null)
          : null;
      case "parent":
      ...
      case "regex": {
        const match = String(current ?? "").match(
          new RegExp(String(args[0]), String(args[2] ?? "i")),
        );
        if (!match) {
          return null;
        }
      ...
      case "matches":
        return new RegExp(String(args[0]), String(args[1] ?? "i")).test(
          String(current ?? ""),
        );
      case "contains":
      case "includes":
      ...
      case "at":
        return Array.isArray(current) || typeof current === "string"
          ? current.at(Number(args[0]))
          : undefined;
      case "length":
        return current != null && typeof current.length === "number"
          ? current.length
          : 0;
      case "slice":
        return Array.isArray(current) || typeof current === "string"
      ...
      case "map":
        return Array.isArray(current)
          ? current.map((item) => this.resolveArg(args[0], item))
          : [];
      case "arrayFind":
        return Array.isArray(current)
  ...
  urlParam(value, name) {
    const results = new RegExp(`[?&]${String(name)}=([^&#]*)`).exec(
      String(value ?? ""),
    );
    if (results === null) {
      return null;
