...
async function loadStatus() {
  ...
  setText(
    statsEl,
    `${info.chapterCount?.toLocaleString()} chapters read on Comick`,
  );
...
function renderUpdateBanner(update) {
  ...
  setText(
    updateCopyEl,
    `v${update.latestVersion} is available. You’re on v${update.currentVersion}.`,
  );
}

function renderQueue(q) {
  pendingList.innerHTML = q
    .slice(0, 15)
    .map(
      (i) => `
        <div class="entry">
...
function renderLibrary(items) {
  ...
  const filtered = q
    ? items.filter((i) =>
        (i.md_comics?.title ?? i.md_comics?.slug ?? "")
          .toLowerCase()
          .includes(q),
      )
    : items;
...
exportSettingsBtn.addEventListener("click", async () => {
  ...
  Object.assign(document.createElement("a"), {
    href: url,
    download: "comicksync-settings.json",
  }).click();
